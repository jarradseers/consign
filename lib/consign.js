...
Consign.prototype._setLocations = function(parent, entity, push) {
  ...
  var ignoreRegex = new RegExp(this._options.ignore + '.*(' + this._options.extensions.join('|') +')', 'gi')
    , filename = path.basename(location)
  ;
    
  if (this._options.ignore && ignoreRegex.test(filename)) {
    this._log(['!', this._['Ignoring file'], ':', filename]);
    return this;
  }
